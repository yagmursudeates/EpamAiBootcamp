  ...
  return (
    <div className="space-y-10">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-foreground tracking-tight">My Ideas</h1>
        <a
          href="/submit"
          className="inline-flex items-center justify-center rounded-lg text-sm font-semibold text-primary-foreground bg-primary h-9 px-4 shadow-sm hover:bg-primary/90 transition-colors"
        >
          + New Idea
        </a>
      </div>
      ...
      {drafts.length > 0 && (
        <section>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {drafts.map((idea) => (
              <div key={idea.id} className="group flex flex-col gap-2">
                <IdeaCard idea={{ ...idea, submitter_name: undefined }} href={`/ideas/${idea.id}/edit`} />
                <div className="flex justify-end gap-1 h-7 opacity-0 group-hover:opacity-100 transition-opacity">
                  <a
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {submitted.map((idea) => (
              <div key={idea.id} className="group flex flex-col gap-2">
                ...
                <div className="flex justify-end h-7 opacity-0 group-hover:opacity-100 transition-opacity">
                  <DeleteIdeaButton ideaId={idea.id} size="sm" />
                </div>
              </div>
            ))}
